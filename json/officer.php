...
if (isset($_SESSION['login'])) {
    ...
    $sql = "SELECT * FROM users WHERE {search} AND role IN ({role}) AND id <> {current_user_id} ORDER BY created_at DESC LIMIT 10 OFFSET {page}";
    $role = '';
    ...
    $sql = str_replace('{role}', $role, $sql);
    $sql = str_replace('{current_user_id}', $currentUser['id'], $sql);
